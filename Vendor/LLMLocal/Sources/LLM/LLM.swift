...
public actor LLMCore {
    ...
    public init(model: Model, path: [CChar], seed: UInt32, topK: Int32, topP: Float, temp: Float, repeatPenalty: Float, repetitionLookback: Int32, maxTokenCount: Int) throws {
        ...
        // ПАТЧ Sage: батч ограничен (prefill чанками), а не равен всему контексту (иначе гигантские буферы).
        contextParams.n_batch = min(contextParams.n_ctx, 512)
        // ПАТЧ Sage: оставляем запас ядер под UI; тяжёлая работа уходит на GPU (Metal).
        let cappedThreads = min(processorCount, 6)
        contextParams.n_threads = cappedThreads
        contextParams.n_threads_batch = cappedThreads
    ...
    func prepareContext(for input: String) -> Bool {
        ...
        // ПАТЧ Sage: префилл чанками по n_batch — иначе llama_decode при промпте > n_batch
        // срабатывает по ассерту (n_tokens ≤ n_batch) и аварийно завершает процесс (ggml_abort).
        let maxBatch = max(1, Int(params.n_batch))
        var offset = 0
        while offset < initialCount {
            let end = min(offset + maxBatch, initialCount)
            clearBatch()
            for i in offset ..< end {
                addToBatch(token: tokens[i], pos: currentTokenCount + Int32(i), isLogit: i == initialCount - 1)
            }
    ...
    func generateResponseStreamWithThinking(from input: String, thinking thinkingMode: ThinkingMode = .none) -> (thinking: AsyncStream<String>, response: AsyncStream<String>) {
        ...
        Task {
            // ПАТЧ Sage: каждый запрос самодостаточен (полный промпт передаётся целиком,
            // history очищается в InferenceService) — чистим KV-кэш и позиции перед префиллом,
            // иначе currentTokenCount копится между запросами и переполняет ctx → пустые ответы.
            resetContext()
...
open class LLM: ObservableObject {
    ...
    public init?(
        ...
        #else
        // ПАТЧ Sage: офлоэд всех слоёв на GPU (Metal). Без этого инференс шёл 100% на CPU → перегрев и лаги.

...
### 🤖 本地 AI
大语言模型（通过 Apple [MLX](https://github.com/ml-explore/mlx) 运行的 Qwen3）在 Apple Silicon GPU 上运行。按你的设备选择大小 —— 轻量 **1.7B**、均衡 **4B**，或最高质量的 **8B**。模型在应用内下载一次，之后完全离线工作。
...
### ⬇️ 空中更新
Sage 会从本仓库的 [Releases](../../releases) 自动保持更新 —— 启动时与每天检查，后台下载，**校验 SHA-256 校验和**，并在下次重启时应用更新。在 **设置 → 更新** 中可切换**自动更新**。参见 [空中更新如何工作](#空中更新如何工作)。
...
- 下载你选择的 AI／语音模型（一次），
- 检查 GitHub Releases 中的应用更新，以及
- Git 同步 —— 仅当你启用时，**只**与你配置的仓库通信。

## 技术

原生 **SwiftUI** · Apple **MLX**（Qwen3 大模型）· **whisper.cpp** 语音转文字 · **CodeMirror 6** 编辑器 · 磁盘上的纯 Markdown · 多模块 [Tuist](https://tuist.io) 工程 · 自研轻量空中更新器（GitHub Releases + SHA-256）。

...
### 🤖 Локальный ИИ
Языковая модель (Qwen3 на Apple [MLX](https://github.com/ml-explore/mlx)) считается на GPU вашего Apple Silicon. Модели разложены по понятным классам — **лёгкая 1.7B**, **стандартная 4B**, **мощная 8B** — каждая в классической сборке и в **DWQ**-версии (дистиллированная квантизация: заметно ближе к полной точности при том же размере и скорости); рекомендуемая — **Qwen3 4B Instruct**. Скачивайте, переключайте и **удаляйте** модели прямо в настройках. Память модель занимает только пока отвечает: веса загружаются по требованию и выгружаются сразу после ответа — в покое Sage остаётся лёгким. Всё скачивается один раз внутри приложения — дальше работает офлайн.

### ☁️ Облачный ИИ по желанию (DeepSeek)
Нужен «мозг» побольше? Вставьте свой **API-ключ DeepSeek** в **Настройках → ИИ-модель**, выберите модель из списка ключа — и чат с инлайн-ИИ пойдут через облако, а локальная модель автоматически подстрахует при любой ошибке. Ключ хранится в шифрованном файле, привязанном к вашему Mac, и никуда не отправляется; удалите его в любой момент — и Sage снова полностью локальный. Подпись в футере сайдбара всегда показывает, какой движок отвечает.
...
Sage обновляется сам из раздела [Releases](../../releases): проверяет новую версию при запуске и раз в сутки, качает её в фоне, **сверяет контрольную сумму SHA-256** и устанавливает при следующем перезапуске. Автообновление включается в **Настройках → Обновления**. После обновления Sage один раз покажет окно «Что нового» с главным из релиза — на языке интерфейса. Подробнее — [как это работает](#как-работает-автообновление).

### 🧰 Мелочи, которые радуют
«Показать в Finder» в контекстном меню файла · дип-линки (`sage://open?path=…`) открывают заметку из других приложений — так работает кнопка «Открыть в Sage» в Ember — с раскрытием дерева до файла · новые заметки создаются пустыми · двойной пробел ставит точку, как везде в macOS.
...
- обратиться к облачному ИИ — **только** если вы сами подключили ключ DeepSeek (по умолчанию выключено).

Секреты (Git-токен, API-ключ) лежат в **шифрованном файле**, привязанном к этому Mac, — они не синхронизируются и никуда не загружаются.

## Технологии

Нативный **SwiftUI** · Apple **MLX** (модель Qwen3) · **whisper.cpp** для речи · редактор **CodeMirror 6** · обычный Markdown на диске · многомодульный проект на [Tuist](https://tuist.io) · облако **DeepSeek** по желанию (свой ключ) · собственный лёгкий апдейтер (GitHub Releases + SHA-256).
